...
(
    ...
    , MIN(wbc) as wbc_min
    , MAX(wbc) as wbc_max
    FROM `physionet-data.mimic_ed.edstays` ed
    LEFT JOIN `physionet-data.mimic_derived.complete_blood_count` le
        ON le.subject_id = ed.subject_id
        AND le.charttime >= DATETIME_SUB(ed.intime, INTERVAL '0' HOUR)
        AND le.charttime <= DATETIME_ADD(ed.outtime, INTERVAL '0' HOUR)
    GROUP BY ed.stay_id
...
FROM `physionet-data.mimic_ed.edstays` ed
LEFT JOIN cbc
    ON ed.stay_id = cbc.stay_id
LEFT JOIN chem
    ON ed.stay_id = chem.stay_id
LEFT JOIN diff
    ON ed.stay_id = diff.stay_id
LEFT JOIN coag
    ON ed.stay_id = coag.stay_id
LEFT JOIN enz
    ON ed.stay_id = enz.stay_id
